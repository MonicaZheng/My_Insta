 ...
 *
 *    Likes
 *每次click button，就会触发submit-like的click event，
 然后会trigger indexjs里的ajax call,那个call会hit addlike方法，
 之后创建db Like数据或者删除，
 之后addlike会return result给create function through success或者error callback，
 然后create再调用callback更新ui
 */

function create_like(success_cb, error_cb) {
  // get the post's id you click
    var post_pk = $(this).siblings('.hidden-data').find('.post-pk').text();
    console.log(post_pk);
//
    $.ajax({
        type: "POST",
        // go to url /like, do addLike function
        url: '/like',
        data: {
            post_pk: post_pk
        },
...
// find the html has 'submit like' and call the function create_like
